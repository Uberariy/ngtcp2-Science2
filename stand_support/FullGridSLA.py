...
if __name__ == "__main__":
    ...
    path_to_results = Path("perf/sla_p1/")

    '''Ensure, that folder to save experiments exists'''
    if not os.path.exists(path_to_agent / path_to_results):
        os.makedirs(path_to_agent / path_to_results)

    '''CONFIG: Change parameters grid below'''
    selection_rtts = [10, 20, 40, 50, 75, 100]
    selection_bws = [40, 80, 120, 160, 200, 230]
    selection_losses = [0.001, 0.01, 0.1, 0.5, 1, 2, 4] # [0.002, 0.005, 0.075, 0.5, 0.00125, 0.0015, 0.01, 0.015, 0.02, 0.1, 0.2, 0.4, 0.05, 0.15, 0.0025, 0.0075, 0.025, 0.03, 0.04, 0.003, 0.004, 0.0125, 0.3, 0.001, 0.75, 0.25, 3, 1.5, 2, 1, 1.25, 2.5]
    ...
    timeConstraint = 30 # If something goes wrong during experiment

    expN = 0
    for cur_rtt in selection_rtts:
        for cur_bw in selection_bws:
            for cur_loss in selection_losses:
                for cur_jitt in selection_jitts:
                    answ_speeds = []
                    answ_losss = []
                    answ_rtts = []
